...
print("=" * 60)
print("ALGORITMO DE PRIM - ÁRVORE GERADORA DE CUSTO MÍNIMO")
print("=" * 60)
...
nx.draw_networkx_nodes(G, pos, node_size=800, node_color='lightblue', edgecolors='black', linewidths=2)
nx.draw_networkx_labels(G, pos, font_size=14, font_weight='bold')

nx.draw_networkx_edges(mst, pos, width=3, edge_color='green', alpha=0.8)

edge_labels = nx.get_edge_attributes(G, 'weight')
nx.draw_networkx_edge_labels(G, pos, edge_labels=edge_labels, font_size=10)

plt.title(f"Árvore Geradora Mínima (Prim)\nCusto Total: {total_cost:.1f}", fontsize=16, fontweight='bold')
plt.axis('off')
plt.tight_layout()
plt.show()
